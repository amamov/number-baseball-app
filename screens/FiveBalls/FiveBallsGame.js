...
const FiveBallsGame = ({
  ...
  const [problem, setProblem] = useState(problemFactory(5));
  ...
  const inputNumber = (key) => {
    if (key === deleteKey) {
      ...
      } else if (checkCountValue(output, "", 4)) {
        setOutput((prev) => ({ ...prev, one: "" }));
      }
    } else {
      if (checkCountValue(output, "", 5)) {
        setOutput((prev) => {
        ...
        setOutput((prev) => {
          if (checkCountValue(prev, key, 0)) return { ...prev, five: key };
          else return prev;
        });
      }
    }
  ...
  return (
    <Container>
      <Output>
        ...
        <OutputBox>
          <OutputText fontLoaded={fontLoaded}>{output.five}</OutputText>
        </OutputBox>
      </Output>
...
export default connect(null, mapDispatchToProps)(FiveBallsGame);
